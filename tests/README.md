...
Core tests take longer than any other FreeDomCrypto tests, due to the high amount of computational work involved in validating core components.
...
To run only FreeDomCrypto's core tests (after building):
...
To run only FreeDomCrypto's crypto tests (after building):
...
```
FreeDomCryptod --regtest --offline --fixed-difficulty 1
```
...
Open the wallet file with `FreeDomCrypto-wallet-rpc` with RPC port 18083. Finally, start tests by invoking ./blockchain.py or ./speed.py
...
To run only FreeDomCrypto's hash tests (after building):
...
To run only FreeDomCrypto's performance tests (after building):
...
To run only FreeDomCrypto's unit tests (after building):
